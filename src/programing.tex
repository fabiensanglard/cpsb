...
The pipeline ingests the code (\icode{.s}/\icode{.c}) written by programmers, the samples (\icode{.wav}) and musics (\icode{.mvg}) from the musicians, and the graphics assets (\icode{.png}) produced by the artists. The outputs are four sets of ROMs ready to be burned on EPROMs. 
...
Some dependencies in the graph make a lot of sense. The Z-80 source code (\icode{.c} and \icode{.s}) only impacts the Z-80 ROM. Likewise, the 68000 code (also \icode{.c} and \icode{.s}) ends up in the 68000 ROM.
...
During the linking stage, all the variables and functions were given an address in either ROM or RAM. The easier symbol to deal with was function \icode{f} which goes in ROM and points to its own instructions.
...
Symbol \icode{va} is interesting because it is readable but also writable. The linker will have assigned a RAM address. Since it is unitialized, it point to whatever is there in the RAM when it started.

Symbol \icode{va} is even more interesting. Like \icode{va} is is readable and writable but its initial value should be \icode{0}. The linker can make \icode{va} point to RAM and even group zero-initialized variable together but setting the value to zero cannot be done. This is something the bootstrap will have to somehow take care off.
...
Since C cannot be used, both bootstraps (one for Z80 and one for M68k) will be written using assembly and bundled in a file named \icode{crt0}.
...
The z80 read the latch in the interrupt, append it into a circular buffer. Interpretation happen in its \icode{main} thread where a byte value \icode{FE} means that the next byte is the ID of a music that should start playback. 
...
Notice how the encoder injects \icode{0xFF} no-op bytes after each write to the latch and how the decoded ignore them.
