...
Because these screens were meant to be used in TV sets consuming NTSC broadcast, the two imperatives were to be close to \textbf{59.95 Hz} vertical refresh rate and \textbf{15,625 KHz- 15,720 KHz} horizontal refresh rate. 

DRAWING TV NTSC -> 59.95 Hz and 
\pagebreak

The Horizontal frequency and Vertical frequency formulas are used to verify the signal generator's  closeness to the NTSC norms are as follows.
...
Before looking at Capcom choices for these three values, let's look at the choices made by the video designer for systems contemporary to the CP-System. The Neo-Geo entry shows a machine using a resolution of \icode{384x264} and a dot-clock of 6 Mhz which means it issues a color every  $\frac{1}{6,000,000}\mathtt{th} $ of a second.
...
\begin{tabularx}{\textwidth}{Xrrr} 
  ...
  \toprule    
   ...
   \textbf{dots} & \texttt{ 427 } & \texttt{ 384 }  & \texttt{ 341 } \\ 
   \textbf{dot-clock (Hz)} & \texttt{ 6711647 } & \texttt{ 6000000 }  & \texttt{ 5369318 } \\ 
\toprule    
   \textbf{HSYNC frequency (kHz) } & \texttt{ 15718 } & \texttt{ 15625 }  & \texttt{ 15745 } \\ 
   \textbf{VSYNC frequency (Hz) } & \texttt{ 59.99 } & \texttt{ 59.18 }  & \texttt{ 60.09 } \\ 
\toprule    
...
As the table shows, all manufactures selected different values for the dot-clock, the number of lines, and the number of dot per line but they all made sure they were very close to the vertical and horizontal frequency a CRT should support.


\subsubsection{Capcom video signal choices}

The CP-System uses a resolution of \icode{512x262} with a dot-clock of 8 Mhz. This figure is not a random value. In order to spare an oscillator, Capcom engineers simply used the CPS-A and CPS-B 16 MHz and halved it for the video system.
...
\begin{tabularx}{\textwidth}{Xr} 
  ...
  \toprule    
   ...
   \textbf{dot clock (Hz)} & \texttt{8000000} \\
\toprule    
   \textbf{HSYNC frequency (kHz)} & \texttt{15625} \\
...
Besides the vertical and horizontal "rules", Capcom engineers had additional constraints. First of all, because the GFX system is digital, they had to pick dimension that could be represented as integer. There was also condition dictated by the graphic system which required both axis to be multiple of 8 (for the tilemaps we will study in the next section).

\subsubsection{Blanking}

The CP-System resolution of \icode{512x262} seem to indicate a very high-resolution for the time. But not all the lines and all the dots on a line can be used, some have to be sacrificed to solve two problems. 

There is the first, obvious problem of retracing vertically and horizontally which are not instantaneous. While the cannon position resets horizontally or vertically, I would leave a trace so no electrons can be shot during that time.

There is a second problem, mechanical this time, where the electron beam wobbles for a bit afterward being reset in a nearly instantaneous fashion. This cost a few more lines and dots to be unused while the beam stabilizes.

Three: Background operations are needed!

As a result, the video designer must perform a second round of decision. How many lines to give to VBLANK, and how many dot give to HBLANK. These decision results in the actual resolution of the machine while the number we studied so far are called "overscan resolution".


\subsubsection{Capcom second set of video signal choices}

Out of these 262 lines available, Capcom decided to use only 224 and let VBLANK last for $\mathtt{262 - 224 = 38}$ lines. Likewise, they kept 383 dot usable per lines and gave $\mathtt{512 - 384 = 128}$.

Every machines sacrified some real-estate to VBLANK and HBLANK.
...
\begin{tabularx}{\textwidth}{Xrrrr} 
  ...
  \toprule    
   \textbf{Usable lines} & \texttt{224} & \texttt{} & \texttt{} & \texttt{} \\
   \textbf{Usable dots} & \texttt{384}  & \texttt{} & \texttt{} & \texttt{} \\
...
% Similar constraints come into consideration when choosing an horizontal resolution. Alike VSYNC, HSYNC induces wobbling which must be hidden by blanking the ends of a line. Horizontal overscan must also be factored in.
...
% Ideally, the proportion of horizontal points versus the number of lines would result in an aspect-ratio of 4:3 perfectly matching the CRT dimensions. In this case, there would be no distortion when the signal is rendered on the physical screen. A property called "square-pixels" which few system managed to achieve. 

% Keeping in mind that regardless of how many lines are BLANKed, the density on the screen is a constant (drawing less lines than 262 does not increase the space between lines), designers must now choose the frequency of color changes, called dot-clock. BALBLA in order render pixel-aspect-ratio PAR of 1:1. .


\subsubsection{PIXEL PAR PART}
All this explaining about video counters was not just pedantic. 

To check all the boxes, Capcom had the choice between only two resolutions, 288x216 and 320x240. Ultimately they picked neither. They gave 38 lines to VBLANK and kept 224 active lines\cite{petitCRT} (BUT COULD HAVE BEEN 240 later). Horizontally, they left 128 elements to HBLANK and kept 384 active ones.  
...
The result is that the CPS-1 does not have square "pixels". It has wide rectangular "pixels" which was a huge problem for artists. If they digitized their drawing as is, the CRT would present to players a vertically compressed version of their original vision.
...
Capcom thoroughly tested their games to make sure the OBJ budget was never exceed. Yet some games such as Final Fight shipped with scenes going beyond the CP-System capacities. In the XX levels, our two heroes encounter an unprecedented level of opposition where the list of sprites is as follows.

\begin{itemize}
\item Two heroes Haggar and Cody.
\item Three barrels.
\item Three Axl (grey heavy) (two temporarily KOed).
\item Three Slash (copper heavy) (two flying!).
\item One Bred (grey minion) and one Dug (red minion).
\end{itemize}

Except Haggar is nowhere in sight behind Cody. Instead we see a blank space. More than 256 tiles are requested and draw commands at the end of the list are dropped. This lasts a few seconds until some of the opponents are neutralized\cite{ffoverload}.
