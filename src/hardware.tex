...
These CRTs of the early 90s were meant to TV sets consuming NTSC broadcast so the two imperatives were to be close to \textbf{59.95 Hz} VSYNC rate and \textbf{15,625 Hz} HSYNC rate. 
...
There is the first problem of retracing vertically and horizontally. The cannon movement is not instantaneous so while it moves horizontally or vertically, I would leave a visible diagonal of electrons across two scanlines in case of horizontal reset or across the whole screen in case of a vertical reset.

The second problem is a matter of wobbling. Because a reset changes the cannon position abruptly (as opposed to the smooth progressioin during a scanline), it takes a little of time for the electron beam to stabilize again after it completes the reset.
...
An "ideal" system would have "square" dots with a 1:1 PAR. For CRTs built with a set physical scanline height square pixels were guaranteed if the dot clock was 6,136,363 Hz ($\frac{135}{22}$). A system using a higher frequency would draw narrower dots while a system using a lower frequency would draw wider dots.
...
\begin{figure}[H]
\img{metalslug.png}
\caption*{Metal Slug as described in the Neo-Geo GFXRAM (320x224)}
\end{figure}
...
The Neo-Geo MVS, with its dot-clock of 6,000,000 Hz has a PAR of 45:44. Combining its \icode{320x240} Storage Aspect Ratio (SAR) with its SAR gives the Display Aspect Ratio (DAR) of the physical image seen on the CRT.

\begin{align*}
 \mathtt{Display\;Aspect\;Ratio = DAR * SAR = \frac{(45*320)}{(44*224)} = 1.46}
\end{align*}

The near-square pixels result in minimal distortion when the image is presented on the CRT. This is very convenient for artists  since they can digitize their assets 1:1 and see their artwork rendered as intended.
