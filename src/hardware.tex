...
\section{Physical Architecture}
 The CPS-1 is made of three stacked Printed circuit boards (PCB). The biggest board at the bottom is Board "A". In the middle is the Board "B". On the top is the smaller Board "C". Boards "A" and "B" are connected via four white connectors featuring 2x32-pin each. Similarly, boards "B" and "C" are connected respectively two white 2x20-pin connectors.
...
For each of these layers, the GFXROM contains no tiles but instead bytecode dictating the position of points as long as palette cycling timing. By using the 32 palettes of each of these layers astutely, parallaxed starfield can be rendered. 
...
The STARs layers also proved useful when designers need a full black background. Instead of using a SCROLL and repeating a tile full of black tiles, they only have to use a STAR and request no stars on it. So many player starred at a dark night sky and they never knew about it.
...
The OBJ system was used in a creative way by akiman to solve a showstopper bug in a title intended to USA market.
...
The 16 tiles making the "World Warrier" title as they are drawn by the CPS-A and CPS-B.
...
Akiman first noticed that replacing the three last tile with tiles \icode{6} and \icode{7}. It reduced the problem by spelling "The World Warrlor". 

\nbdraw{world_warrWor}

\begin{figure}[H]
\img{sf2_title_warrier.png}
 \caption*{A recreation of the fear inducing typo. Ouch!}%
 \end{figure}%

Now all Akiman had to do was to somehow find a way to turn this 'l' character into an 'i'! That would have been simple if the CPU could have written in the VRAM but it could not.

There is a way to fake pixel plotting. It is very expensive. The idea is to find a tile almost fully transparent but with only a single pen value in it. Akiman found that Guile's calve met these criteria.

Guile's calve is drawn three times across the 'l' to split it into an 'i'.
...
% \begin{figure}[H]
%  \img{sf2_title.png}
%  \caption*{The tile screen as it appears!}%
%  \end{figure}%
