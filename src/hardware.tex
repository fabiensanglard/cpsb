...
\subsection{Board A}\index{Board A}
...
\subsection{Board B}\index{Board B}
...
\subsection{Board C}\index{Board C}
...
\subsection{Z-80}
Released in July 1976 by Zilog, the Z-80 was an intended as an Intel 8080 killer. It ended up sharing the front-scene from the late-70s until the mid-80s with the equally mythical MOS 6502. 
...
Thanks to its 8-bit design, the z80 was a perfect fit for the 8-bit YM2151 and 8-bit MSM6295. Additionally, having been around for a while, the outdated CPU was inexpensive. Equally important, Zilog's CPU enjoyed a good reputation from programmers who enjoyed its simple programming interface.
...
Although it is capable of relinquishing control of the bus via \icode{BUSRQ} Request bus mastering,\icode{BUSAK}, Acknowledge, \icode{MREQ} Request to use bus, and IO bus \icode{IORQ}, the Z-80 completely own its bus and never shares it. 
...
Little RAM is needed by the Z-80 since it forwards request from the latches to the OKY6295 and feeds the YM2151 music notes. A  single 2Ki x 8-bit \icode{CXK5816SP} chip is used. The ROM is made of a 64Ki x 8-bit \icode{27C512} chips. It is large to store FM musics along the z80 instructions. 
...
\begin{trivia}
Hopefully this awful bank switch control byte a programmer must juggle with to access data located beyond \icode{0x8000} will leave no doubt with regards of the awesomeness of the 68000 and its 24-bit address space. 
\end{trivia}
...
\begin{trivia}
The CPS-1 layer system does not use a painter algorithm where pixels are written over and over in a framebuffer. The CPS-B receives a stream of all layers and based on the priority and the potential transparent pen value (\icode{0xF}) select the one to send to the video system.
\end{trivia}
