...
This system is so powerful that the entire Neo-Geo rendering pipeline revolves around its double line buffer system. It need no tilemap system.
...
Finally, the scrolls of frame 3 are displayed but along with the spritebuffer from frame 2. The desycing can only be compensated in software.
...
The ultimate boss in "The Punisher", Kingpin, is a mountain of a man made of 69 tiles which covers half the screen. This impressive feat came without minimal "wasted" pixel thanks to the usage of non-rectangular shapes.


Tiles in the OBJ layer have attributes allowing them to be rendered horizontally and/or vertically flipped. However, once again, there is not support for rotation or scaling.
...
\end{minipage}
 ...
 When Final Fight min-boss, Damned, does a back-flip in level 1, no rotation is performed. Two sets of tiles are used and X flip/Y flipped to generate two extra mirrored sets. The effect works with four poses, thanks to fast movement and players' brain interpolation.
...
The sprite system has a hard limit of 256 tiles. This is not an arbitrary number, the constraint is dictated by how many tile the system is able to read from the GFXROM and write to the VRAM during a full raster scan (16.7ms).
...
\end{q}
To remain within the OBJ budget, the "wind, forest, fire, mountain" ("\begin{CJK}{UTF8}{min}風林火山\end{CJK}") sign was removed. All breakable (Ken's barrel, Guile's crate and Dictator's statues) were allowed to remain.
