...
To fake it, two logos are used, a small (beginning) and a large one (final). Tiles move on a circular pattern revolving around the center of the shape. The small to large substitution happens at the end of the first revolution.
...
\img{sf2_title_warrier.png}
 \caption*{A recreation of problem}%
 \end{figure}%
...
The problem was displaced from turning an 'i' into an 'o', to turning an 'l' into and 'i'. That would have been simple if the CPU could have written in the VRAM but as we have seen these chips are not mapped in the m68k address space.

There is an expensive way to fake pixel plotting. The idea is to find a tile almost fully transparent (\icode{0xF}) but with only a single pen value set in its. 

Akiman found that in one of Guile's pose, his calve met these criteria with a simple pen value in its lower left.

\begin{figure}[H]
\nbdraw{guileCalve}
\caption*{Guile's calve saves the day}%
 \end{figure}%
...
A quick glance at the \icode{HM53461P} shows the usual \icode{+5V}, \icode{GND}, \icode{CLK}, and address/data pins. However \icode{SD} pins are visible, indicating this chip does more than the one we have seen so far.
...
It is likely the dual-channel architecture is the result of a desire to use inexpensive components to keep the price down.
...
Capcom's response was a two-way verification where the hardware that could be actively used by the software to check the board authenticity backed by "passive" mechanisms that allowed the hardware to check the software behavior.
...
Starting with CPS-B v21, a slightly more robust feature gave the CPS-B the ability to perform multiplications. Two registers are written and a third one can be read. The 68000 code is expected to check that the returned multiplication result is the expected value.

\subsection{Moving registers}
The CPS-Bs registers move between revisions. They 68000 memory map of the CPS-B registers does not change, but the offset inside that mapping is altered. Accessing the scroll control, scroll priority, and palette upload registers is done using a different offset for each game.

Additionally, the meaning of each bit inside each register is altered between version.
...
The whole behavior is encoded in a small internal 18 bytes area using not ROM but a RAM. To keep these bytes alive, the CPS-B v21 must be supplied with current at all time\cite{petitSecurity}.
...
People phoenixing CPS-1 board have such intimate knowledge of the CP-System that they even changed the splash screen of the game to feature a "Phoenix Edition" icon.
...
Eventually, passionate people figured out the process of accessing the CPS-B RAM. Boards can be brought back to life by re-uploading the proper configuration bytes which essentially de-suiciding them\cite{arcadeHackerCPS1Desuicide}.
...
To counterfeiters, the CP-System was a problem. Capcom games were popular and generated a substantial amount of money. It is likely greed set even more once the demand skyrocketed with the advent of Street Fighter II. 
...
Of all the security measures, it would have been fair to assume the custom ASICs would be an impenetrable fortress. Astonishingly, CPS-A and CPS-B replicas were manufactured under the name "COMCO"\cite{arcadeHackerCPS1}. It is unknown if an insider leaked the schematics or if someone made it their life mission to reverse-engineer these chips to make it happen.
...
From 1993 to 2003, forty-two games were published. The first one was the successful "Super Street Fighter II" while the last one "Hyper Street Fighter II: The Anniversary Edition" paid homage to a series that defined the platform.
...
The only part of the CPS-2 that remain stored in plain is the OKI ROM containing audio samples. The GFXROM data was not encrypted but the content was shuffled.
