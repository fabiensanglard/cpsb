...
A JAMMA port centralizes everything a game needs to operate. The port features 28 pins on each sides to provide inputs, outputs, and power supply. Among the inputs are a four-direction joystick and three buttons per player, two coin sensors, start button, and service button. On the list of outputs, mono speakers lines and CRT screen controls.
 ...
 \begin{figure}[H]
\img{jamma.png}
\caption*{JAMMA solder side pins}
\end{figure}

 \begin{figure}[H]
\img{jamma.png}
\caption*{JAMMA chip side pins}
\end{figure}
...
% There are a lot of chips and part but they can be broken down in sub-systems. Hopefully it should all make sense by the end of this chapter
...
The two chips are not aware of each others. They are connected to the same 14 address lines and the same control lines for write (\icode{WE}) and read (\icode{OE}). However, they are connected to different lines of the data bus. 
...
The 68000 instructions are provided by \icode{27C010} chips which are 128 KiB x 8-bit chips. They work exactly like the \icode{65256BLSP-10} except that they have more address lines.
...
In order to place one pair after another in memory space, the \icode{CE} (Chip Enabled) (sometimes labeled \icode{CS} for "Chip Selected") pin is leveraged. Asserting it let a chip respond to address request while de-asserted keep it dormant.
...
The first pair of chips is mapped to addresses \icode{0x000000} while the second pair is mapped to \icode{0x40000}. With the same logic, two more pairs of \icode{27C010} are mapped at \icode{0x80000} and \icode{0xFFFFF} for a total of 1 MiB ROM.
...
Game configuration is retrieved from DIP switches. The game start where an infinite loop polls coins sensors and players inputs and generates video and audio outputs in response. 
...
Little RAM is need by the Z-80 since it forwards request from the latches to the OKY6295 and feeds registers  of the YM2151 for music. The RAM is is a single 2Ki x 8-bit \icode{CXK5816SP-15L}. The ROM is a single 64Ki x 8-bit \icode{D27C512} which is larger to store musics on top of Z-80 instructions. 
...
To work, the OKI only needs a sample ID [1-127], a channel [1-4], and a volume. Then, via a lookup table in ROM the sample offset is retrieved and playback starts with an analog signal generated on \icode{DA0}. 
...
Clockable between [1MHz-5MHz] with two modes, the goals was to maximize quality while minimizing required storage. The table shows that the best quality (37kHz) only allowed to store 13 seconds of samples while the worse (6kHz) gave 69 seconds.
\begin{figure}[H]
{
...
\begin{figure}[H]
\nbimg{latency.png}
\caption*{Backgrounds and Sprites are out of sync. Always.}
\end{figure}
...
A glance at the \icode{HM53461P-10} chips shows that there is not here than the "simple" data, address, and control we have seen so far.
...
Instead of the \icode{27C010} (128 KiB x 8-bit) chips, we find MB834200B-15 (512 Kib x 8-bit) with 4x times the capacity. To make up for the access time, the chips are arranged in a four-way interleave resulting in a 32-bit data path.
