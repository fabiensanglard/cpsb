...
Other machines did not have square pixels. It is the case of the Super Nintendo. With its 5.37 MHz dot-clock, it sports a PAR of 8:7.

Combining its \icode{256x224} format gives the aspect ratio of the physical image seen on the CRT.

\begin{align*}
 \mathtt{Display\;Aspect\;Ratio = \frac{(8*256)}{(7*224)} = 1.30}
\end{align*}
...
\begin{figure}[H]
\img{smw.png}
\caption*{Super Mario World as described in the GFXRAM}
\end{figure}
\pagebreak

Non-square pixels were a problem for artists. 


\vfill
\begin{figure}[H]
\img{smw_scaled.png}
\caption*{Super Mario World as it appears on a CRT, PAR = 1.30}
\end{figure}
\pagebreak



The CPS-1 uses non-square dots which stretch the image vertically onto the CRT.

\begin{align*}
 \mathtt{Display\;Aspect\;Ratio = \frac{(135*384)}{(176*224)} = 1.31}
\end{align*}

CPS-1 PAR 135:176 avec la définition 384x224 t'obtiens (135*384) / (176*224) ~= 4/3

The CPS-1 PAR of X:Y means it does not have square "pixels". There wide "pixels" became a huge problem for artists. If they digitized their drawing as is, the CRT would present to players a vertically compressed version of their original vision.
...
Many drawing in the rest of the book square or rectangles are used interchangeably.
