...
To add rainfall, developers leveraged temporal blending on the same layer as the GUI.\label{gg_rain}
...
The introduction sequence of the shoot'em up Carrier Air Wing (page \pageref{caw_color} and \pageref{caw_rgb}) is even more impressive. As a F-14 Tomcat takes off from its carrier, the jet leaves in its trail an exhaust that expands smootly one pixel line at a time. The gaze then proceed to disperse with a fizzelfade effect to let the title logo appear. 

All the elements in this sequence seem to indicate that pixels are plotted by the CPU writing into a framebuffer but all sprites on the OBJ layer (\fcolorbox{black}{black}{\vphantom{W}\hphantom{H}}) are prerendered tiles. The tile repeat pattern is visible on the fourth color coded screenshot.
