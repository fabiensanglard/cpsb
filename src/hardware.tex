...
\section{Goals}
The CPS-1 was expected to solve most of Capcom arcade division problems. Namely reduce production cost, lower selling price, streamline development, increase GFX/SFX/processing capabilities, and stop piracy. 
...
More importantly, games would had to catch customer eyes and not pale in comparison with competition. The goal was to design a machine with capabilities an order of magnitude above the current tech stack able to generate audio and visuals that would hold they own compared to titles from powerhouse such as Sega or Namco.

Finally was the problem of piracy. In a country like Mexico it was estimated that 200,000 bootlegs were in circulation\cite{sf2_oral_history} despite Capcom recording zero sales in that territory. Multiple concurrent copy-protection mechanisms were to be implemented.
...
% \begingroup
Arcade operators frequently updated their cabinets by replacing the game it ran in order to keep on bringing novelty to players and quarters to their pockets. Thanks to the \textbf{J}apan \textbf{A}musement \textbf{M}achine and \textbf{M}arketing \textbf{A}ssociation, the process of updating was simple. 

The belly of these machines usually hid an abomination of tangled wires converging toward a JAMMA harness where the motherboard would be inserted as a slot-in. All an operator had to do was to swap the old with the new PCBs.
...
\section{Physical Architecture}
 The CP-System is made of three printed circuit boards named Board "A", Board "B", and Board "C" which are stacked on top of each others.
...
A summary look at page \pageref{fig:boarda} reveals the powerhouse of the whole system where even an untrained eye will notice the size of an \textbf{A}pplication-\textbf{S}pecific \textbf{I}ntegrated \textbf{C}ircuit (ASIC) and the shear number of bus lines leading to it. In the center left stands the "CPS-A", in charge of 50\% of the graphic system and its 16MHz oscillator. Just above is 384 KiB of VRAM to store a special kind of framebuffer studied in later pages.
...
The middle right part is where the audio system lives. It is made of a Zilog z80, a 3.58MHz oscillator, and 2 KiB of "work" SRAM. Also in this area are present the audio chips dedicated to music (YM2151 and YM3012) and sound effects (OKI6295).
...
\pagebreak
   ...
   There are 3x8 = 24 chips, referred as "GFX ROM", dedicated to storing GFX via sockets [1-8], sockets [10-17], and sockets [20-27] for a total of 12 MiB capacity. Because of the price of ROM, games were never budgeted to allowed the max capacity. Most titles were granted 2/4MiB, three (the Street Fighter II series) were allowed 6 MiB and one (Dynasty Wars) got a whooping 8MiB.
...
Board "C" hosts the "CPS-B" ASICs video chip. It is in charge of the remaining 50\% of the graphic pipeline, namely mix data from the VRAM and the GFX ROMs towards the pixel generator. Capcom also concentrated its anti-piracy measures in that chip and as a result revised the board "C" many more times than the board A and board B. 
...
\sdraw{0.44}{PAL16L8A}
 ...
 Often located near the memory chips group they affect, they are codenamed based on their function. Since most games use slightly different ROM layout, they usually feature different PALs. e.g: Street Fighter II's \icode{STF29} which organizes the GFX ROM is named \icode{S224B} in Final Fight, and \icode{DM620} in Ghouls'n Ghosts.
...
The control system is the master of the platform. As a ruler it needs not to excel at a specific task but to be able to direct and keep tabs on many components. A tailor made task for the Motorola 68000.


\subsection{Motorola 68000 CPU}\index{Processors!Motorola 68000}

 Released in 1979, and clocked at 10 MHz (later upgraded to 12 MHz), the 68000 was not a particularly powerful chip by the late 80' standards with its two stage pipeline\cite{M68000fv} (prefetch, exec) and no internal cache. Its 1.7 MIPS placed it on par with an Intel 286 10MHz (1.5 MIPS). By 1989 it was already two generations old behind the 1984 M68020 (3 MIPS) and the 1987 M68030 (5 MIPS)\cite{mips}.
...
Other CPUs used small address registers resulting in the infamous segmented addressing, Motorola gave its CPU 32-bit data and address registers. The elegant flat addressing and generous eighteen registers make it a favorite among programmers. 
...
The bus is made of \icode{D0-D15} for data and \icode{A1-A23]} for addresses while address ACK (\icode{AS}), Read/Write (\icode{R/W}), \icode{UDS}, \icode{LDS}, and Data ACK \icode{DTACK} are bus control pins.
...
The two \icode{65256BLSP-10} are not aware of each others. They are connected to the same 15 address lines and the same control lines for Write Enabled (\icode{WE}) and Read Enabled (\icode{OE}). However, they are connected to different lines of the data bus.
 ...
 \begin{trivia}
Interleaving chips can also help to beat memory latency by increasing throughput. Early GPU such as the Voodoo 1 and Voodoo 2 by 3Dfx extensively relied on this technique, even using four-way interleave to keep up with the bandwidth requirements\cite{TheStoryOf3Dfx}.  
 \end{trivia}
...
This is an over-simplification to introduce complexity progressively. We will see next how chips are organized to not conflict with each others.
 ...
 \begin{trivia}
 64 KiB of work RAM seems like a lot but that was not always enough. Some games found themselves with not enough RAM and too much GFXRAM. Street Fighter 2 Champion Edition programmers resolved to generating and executing instructions from the GFXRAM\cite{mame_driver}!
 \end{trivia}
\pagebreak
...
The details of the 68000 operations will be studied in-depth in the next chapters but we can already guess how the CPU operates based on what it has access to. As the m68k boots, it starts to retrieve instructions from it ROM. For regular operations such as store/load, and also to keep track of its call stack, it uses its work RAM. 
...
\label{z80_pinRFSH}
The \icode{RFSH} pin (ReFreSH signal) tics at regular intervals to trigger DRAM refreshes. The sound system uses only SRAM so it is not used although this pin was re-purposed in a creative way for the CPS-1.5 "Kabuki" (page \pageref{kabuki}).


\pagebreak

\subsection{z80 Work RAM}
The amount of RAM provided to the z80 may appear scandalously small by today standards. Because all it has to do is forward request from the latches to the MSM6295 and feeds the YM2151 music notes, the z80 needs few resources. Its bus is connected to a single 2Ki x 8-bit \icode{CXK5816SP} chip.
...
The solution is to map only the portion of the ROM that contain instructions (32KiB) statically and to use a banking system to provide a 16 KiB "view" into the remaining 32KiB of the ROM where the music assets are stored. This is accomplished simply with a PAL (\icode{SOU1}) and was the source of great pain to the developers (see page \pageref{memory_bank_programming}).


Hopefully the thought of this awful bank switch control register will leave no doubt with regards of the awesomeness of the m68k and its 24-bit flat addressing system. 
...
A pin is of particular interest to us. \icode{IRQ} allows the YM2151 to generate interrupts based on two internal counters, It's usage is detailed in the programming section.
...
Up to four channels can be active simultaneously. Since games don't need that many sound effects simultaneously, two channels are usually reserved for sound effect playback while two are dedicated to embellishing musics with samples.
...
Running within [1MHz-5MHz] in two modes, the goals was to maximize quality while minimizing required storage. The table shows that the best quality (37kHz) only allowed to store 13 seconds of samples while the worse (6060Hz) gave 86 seconds.
\begin{figure}[H]
{
...
ADPCM is able to take 12-bit PCM samples and compress then as 4-bit nibbles by encoding only the difference between PCM samples. Decompressing an ADPCM stream consist in adding a delta value to the last decompressed  sample, over and over again.
...
\subsection{Syncing}
The RGB signals describe lines to be drawn but the CRT needs to know where to draw them. The control signals allows to synchronize the cannon orientation with the lines color signal so they are rasterized where they should. Without syncing, the image appears distorted. 
...
% The horizontal syncing is done via a HSYNC signal and the vertical syncing is done via a VSYNC signal. These are detected by the CRT to position the gun properly.

VSYNC signal tells the CRT that it should reset the gun's vertical position to 0 at the top of the screen. This motion from bottom to top is called vertical retrace. During the retrace the gun must stop shooting electrons. This is achieved by requesting a black color on the RGB signal. This "blanking" of the RGB lines happens a little bit before and after VSYNC. The total time not drawing anything is called VBLANK.

HSYNC signal tells the CRT that a line has been drawn and the gun horizontal position should be reset to the left of the screen. This motion is called horizontal retrace. Alike the VBLANK, there is a HBLANK timespan. 
...
Since this space is not used for another field, the resulting effect is black horizontal strips on the screen. Note that the problem is compensated by line bleeding so the black lines are not as big as the visible lines.
...
Besides interlacing which had to be avoided, many other decisions had to be made.
...
To craft a video system meant to build a signals generator and a color generator. We'll study the signal generator first. This circuit is built to take an oscillator signal as input and must be able to output one signal to tells for how long the color generator should hold a color on the color lines, one signal triggers HSYNC, and one signal triggers VSYNC. 

\nbdraw{video_signal}

The oscillator feeds an "horizontal register" which upon each tick is increased by one. Upon reaching its max value, the register wraps around to both issue a HSYNC and increases a "vertical register". 
...
A designer can pick any oscillator frequency (which we called dot-clock from now on), however they much be careful to choose vertical max value (number of lines) and horizontal max value (number of dots per line) so the vertical frequency and the horizontal frequency are compatible with what a CRT can sync on. 

These screens were meant to be used in TV sets consuming NTSC broadcast, the two imperatives were to be close to \textbf{59.95 Hz} VSYNC rate and \textbf{15,625 Hz} HSYNC rate. 

\draw{ntsc}


The Horizontal frequency and Vertical frequency are directly derived from the three values picked for the signal generator. Plugging in the numbers into the following formula allows to verify how close a system is from being compatible with a CRT.

\begin{align*}
 \mathtt{Horizontal\; frequency} = \dfrac{\mathtt{dotclock}}{\mathtt{numLines}}
\end{align*}
\begin{align*}
...
Before looking at Capcom choices, let's look at the decisions made by the video designer for systems contemporary to the CP-System. The Neo-Geo entry shows a machine using a resolution of \icode{384x264} and a dot-clock of 6 Mhz which means it issues a color every $\frac{1}{6,000,000}\mathtt{th} $ of a second.


\begin{figure}[H]
{ \setlength{\tabcolsep}{3.0pt}
\begin{tabularx}{\textwidth}{Xrrr} 
  \textbf{ } & \textbf{Genesis} & \textbf{ Neo-Geo }  & \textbf{ Super NES } \\               
  \toprule    
   \textbf{dots} & \texttt{ ? } & \texttt{ 384 }  & \texttt{ 341 } \\ 
   \textbf{lines} & \texttt{ ? } & \texttt{ 264 }  & \texttt{ 262 } \\ 
   \textbf{dot-clock (Hz)} & \texttt{  6,711,647 } & \texttt{ 6,000,000 }  & \texttt{ 5,369,318 } \\ 
\toprule    
   \textbf{HSYNC frequency (Hz) } & \texttt{ ? } & \texttt{ 15,625 }  & \texttt{ 15,745 } \\ 
   \textbf{VSYNC frequency (Hz) } & \texttt{ ? } & \texttt{ 59.18 }  & \texttt{ 60.09 } \\ 
\toprule    

\end{tabularx}%
}\caption*{Signal generators values for Genesis, Neo-Geo, and Super NES}
\end{figure}

Keep in mind that these resolution are not what a programmers can count on. Because of overhead discussed in the next section, some lines and dots are unavailable. The resolution presented here are called "overscan resolutions".


\subsubsection{Capcom video signal choices}

The CPS-1 uses an overscan resolution of \icode{512x262}. The dot-clock is 8 Mhz which is obtained by halving the the CPS-A/CPS-B 16 MHz clock and spares an oscillator chip.
...
\end{tabularx}%
}\caption*{Signal generators values for CP-System}
\end{figure}

Besides the vertical and horizontal frequency "rules", Capcom engineers had additional constraints. Because the graphic system works with tiles (which we will study in the next section) using size of 8, 16, or 32 pens, both axis dimensions had to be multiple of eight.

\subsubsection{Blanking}

The CP-System overscan resolution of \icode{512x262} seem to indicate a very high-resolution for the time. But not all the lines and all the dots on a line can be used, some have to be sacrificed to solve three problems. 

There is the first problem of retracing vertically and horizontally. The cannon movement is not instantaneous so while it resets horizontally or vertically, I would leave a visible diagonal line of electrons across two scanlines in case of horizontal retrace or across the whole screen in case of a vertical retrace.

The second problem to solve is a matter of wobbling. Because a reset pulls the cannon abruptly in the opposite direction, it takes a little of time for the electron beam to stabilize again after it completes the reset.

Lastly, the video system needs break to read or write data without generating visible artifacts. This includes swapping buffers, updating the palettes colors, and retrieve a list of sprites/tilemap to draw on next line.

The solution is named blanking. By setting color signals to zero, the cannon shoots no electrons, artifacts are hidden and a timespan where the video system is inactive is created. There is a vertical blanking called VBLANK and an horizontal blanking called HBLANK.


\subsubsection{Capcom second set of video signal choices}

Out of these 262 lines available, Capcom decided to use only 224 and let VBLANK last for $\mathtt{262 - 224 = 38}$ lines. Likewise, they kept 383 dot usable per lines and gave $\mathtt{512 - 384 = 128}$. These left the machine with an usable resolution of \icode{384x224}.

\begin{figure}[H]
{ \setlength{\tabcolsep}{3.0pt}
\begin{tabularx}{\textwidth}{Xrrrr} 
  \textbf{ } & \textbf{CP-System} & \textbf{Genesis} & \textbf{Neo-Geo} & \textbf{Super NES} \\
  \toprule    
   \textbf{Usable dots} & \texttt{384}  & \texttt{320} & \texttt{320} & \texttt{256} \\
   \textbf{HBLANK (dots )} & \texttt{128}  & \texttt{} & \texttt{} & \texttt{}  \\
   \textbf{Usable lines} & \texttt{224} & \texttt{224} & \texttt{224} & \texttt{224} \\
   \textbf{VBLANK (lines)} & \texttt{38}  & \texttt{} & \texttt{} & \texttt{}  \\
\toprule    
...
The scanlines of a CRT have a fixed height and the dot-clock dictates the width of each dot. Together they define a ratio width/height called the Pixel Aspect Ratio (PAR).

All CRTs at the time were built with physical line height guaranteeing that a dot clock of 6,136,363 Hz ($\frac{135}{22}$) would generate dot with a square PAR (1:1). A system using a higher frequency would draw narrower dots while a system using a lower frequency would draw wide dots.

\begin{figure}[H]
\nbdraw{par}
\caption*{Pixel Aspect Ratio of four systems (exagerated)}
\end{figure}


Machines like the Neo-Geo and the Genesis had a PAR close to 1:1 resulting in square pixels\cite{par}.

\vfill
\begin{figure}[H]
\img{metalslug.png}
\caption*{Metal Slug as described in the Neo-Geo GFXRAM, PAR = 1}
\end{figure}


The PAR formula is a simple division.

\begin{align*}
 \mathtt{PAR = \dfrac{dotclock\;MHz}{\frac{135}{22}} = \dfrac{dotclock\;MHz * 22}{135}}
\end{align*}

The Neo-Geo MVS, with its dot-clock of 6,000,000 Hz had a PAR of 45:44. Combining its \icode{320x240} format gives the aspect ratio of the physical image seen on the CRT.

\begin{align*}
 \mathtt{Display\;Aspect\;Ratio = \frac{(45*320)}{(44*224)} = 1.46}
\end{align*}

The near-square pixels result in minimal distortion when the image is presented on the CRT. This is very convenient for artists  since they can digitize their assets 1:1 and experience minimal difference on screen from they artwork.

\vfill
\begin{figure}[H]
\img{metalslug_4_3.png}
\caption*{Metal Slug as it appears on a CRT, PAR = 1.46}
\end{figure}
...
The CPS-1 with its resolution of \icode{384x224} and its dot-clock of 8,000,000 Hz resulted in a PAR of 135:176. Its output signal somewhat matches the CRT aspect ratio of 4:3. 

\begin{align*}
 \mathtt{Display\;Aspect\;Ratio = \frac{(135*384)}{(176*224)} = 1.31}
\end{align*}

However its narrow pixel result in a significant amount of distortion which was a huge problem for artists. If they digitized their drawing as is, the CRT would present to players a vertically stretched version of their original vision. As illustrated on page \pageref{sf2_ratio}, an artist drawing a circular moon on paper, digitizing it as is and running it via the CPS-1 would see an oval satellite on the screen.
...
Could it has been an oversight? Could it actually been a calculation error? In all likelihood the hardware designers wanted to give the CPS-1 a very high horizontal resolution to make it competitive, even it is meant making artist life a little bit difficult.



In the rest of the book, the format of images will vary. For real-estate reason, the "screenshot" may be shown with 1:1 aspect ratio or 1.3 aspect ratio depending on the needs. The difference is pretty significant (as seen on page \pageref{sf2_ratio_solution}) and they should be minimal confusion.

\pagebreak

\sbimg{0.99}{sf2_intro.png}
\label{sf2_ratio}

\sbimg{0.99}{sf2_intro_4_3.png}
...
The vertical refresh rate of 55.017605 Hz was visually less pleasing and dangerously 10\% off from the CRT recommended values. This refresh rate is difficult to replicate for "modern" emulators but what an impressive feat for a 1987 system!
\end{trivia}



\subsection{Colors} \index{Colors!Space}
...
The sheer amount of black in the drawings show the extend of the overhead associated with beam wobbling management. But the time spent not drawing is not wasted. It is leveraged to perform background operations such as modifying palette colors. e.g: Sixteen lines are necessary to a palette page (32 palettes) "upload".
...
These memory elements features pinouts explained earlier like Power \icode{+5V}, Ground \icode{GND}, Addresses \icode{A0-A10}, Data \icode{D0-D7}, Write (\icode{WE}), Read (\icode{OW}), and Chip Enabled (\icode{CE}).
...
Colors are grouped into palettes containing 16 units. As will be studied later, the GFX system features 6 layers and each of them allows 32 palettes (called page). This brings the total to 6*32*15 = 2,880 colors which requires 12-bit to be indexed.
 ...
 The palette SRAM chips are nearly constantly used to generate colors. Their content can only modified to during VBLANK.
...
Solving these problems of timing and latency well is what made the CPS-1 stand out. It is unarguably the "secret sauce" of the system. 
...
This division make the GFX system unaware of the color that will appear to players on the CRT since it only manipulates pens.
...
A naive design would work at the same speed as the video system (8MHz). For each pixel (every 125ns) the GFXRAM would be read to know what tile to display. Then a pen would be retrieved from the GFXROM. Finally that pen would be sent to the palette system where the color would be converted by the video DAC.
 ...
 Even thought the machine uses the fastest type of memory (SRAM), its response time does not permit that many roundtrips. This problem is solved via caching, streaming, channeling and an humongous (at that time) 32-bit GFXROM/CPS-B local data bus. 


\subsubsection{Caching}
The CPS-A only accesses the GFXRAM during the HBLANK interval. To eliminate memory read operations while a scanline is rasterized, a whole line worth of tilemap is retrieved and stored in an internal cache of 256 entry. Each entry stores 16-bit  tileID + 10-bit attributes. Notice the two parts on the die storing respectively 10 bits and 16 bits.
...
The system works with the GFXROM addresses lines connected to the CPS-A (via with intermediate PAL decoding) but the data lines are connected to the CPS-B where pen values are selected/discarded before being sent to the video system.
...
Each tilemaps has a maximum capacity of 32 palettes (called a palette page) which any tile can use freely. SCROLLS can be offset ("scrolled", hence their name) by any X or Y value, appear in any order and be used for any purpose.
...
Other titles such as shoot'em up Forgotten World required all the sprites the machine could provide for the gameplay. To avoid wasting any of them, the GUI is drawn on SCROLL1 instead of OBJ. The trade-off is that GUI elements are aligned on a 8 pixels grid which is a minor inconvenience.
...
Besides SCROLLs, the CPS-1 has two "STARfields" layers which are always behind the SCROLLs and always in order STAR1 then STAR2. Alike the other layers, one full page of 32 palettes is available to each of them.
...
em up like R-Type, Gradius, or Darius at the time made a good case for it. Providing a system saving both considerable GFXROM space and artists time was a good idea at the time. Ironically, the platform ended up receiving no space shooter! 

That did not prevent the feature to be used. Long after the STARfields came out of fashion, the system was re-purposed.
...
This is the unacknowledged poetry of Street Fighter Hyper Fighting intro sequence. As the title appears, the black background is in fact a pitch black night sky which nobody ever knew about.

\begin{trivia}
The layers system does not use a painter algorithm where pixels are written over and over in a framebuffer. The CPS-B receives a stream of all layers pens which are selected based on the layer priority and transparency value (\icode{0xF}). Once selected, the pen value is forwarded directly to the video system.
\end{trivia}
...
This is how Final Fight characters are sandwitched by SCROLL2. The tiles making the "near" portion of the ramp are tagged to use two priority mask groups. The "wood" tile use the group mask \icode{0} to make pens resulting in colors
...
\begin{q}{Akira Nishitani, Capcom Game Planner}
...
Relegated to implementing flames effects in "Magic Sword" and hazy background in "Carrier Air Wing", rowscrolling barely appeared for a few second of gameplay in the five titles\cite{mame_cps1_video} it was featured in.
...
Besides priority mask, tiles can be flipped horizontally and/or vertically but there is no scaling or rotation. Moreover, the CPU has no access to the VRAM which forbid pixel "plotting". That did not prevent seemingly impossible effects to be achieved.
...
Drawing sprites is more difficult than drawing tilemaps. It involves solving the same problem of bandwidth and latency only sprites can appear anywhere on the screen and are not aligned on a grid.
...
It is an approach that comes with two drawbacks. First, it requires one circuit per sprite which is expensive. Second, the one-to-one complexity make it impossible to scale to more than a few units. Nonetheless, this is the solution used by machines such as the Commodore 64 which advertised their circuitry as "hardware sprites".\index{Sprites Multiplexing!C64}
...
As the CRT cannon progresses down the screen, a sprite unit used above can be reused to draw sprites located. By changing the configuration during HBLANK, many more than eight sprites can be drawn. This trick was extensively in games to reach well over 100 on screen.
...
A linebuffer system requires a buffer as wide as a visible line on the CRT. The buffer is populated with pen codes by a Pixel Processing Unit. The number of sprites, scale and rotation capabilities depends on how much work the PPU is able to do. 
...
This technique is so powerful that the entire Neo-Geo rendering pipeline revolves around its double line buffer system. It need no tilemap system.
...
First, the price of the machine goes up since it requires much more buffering capacity. At the resolution of 384*224, 9 bits per pixel are stored (5-bits palette index + 4-bit color index) requiring 200 KiB of storage for two framebuffer.

\subsubsection{Bandwidth requirements}
The second impact is on the bus. A massive amount of data is now written and read to/from the VRAM. It requires so much bandwidth that an especially large data bus connecting the GFX ROM and the VRAM must be designed.
...
\begin{trivia}
The sync issue is particularly noticeable in Final Fight level 2. The subway wagon moves up and down to simulate rails junction bumps but the handles on the ceiling and the characters appears to lag behind.
\end{trivia}

A three frames sequence is enough to illustrate the issue. On frame 1, the scrolls of frame 1 are displayed. No sprites are visible at that point.
...
Next, the scrolls of frame 2 are displayed but along with the spritebuffer from frame 1.
...
Finally, the scrolls of frame 3 are displayed but along with the spritebuffer from frame 2. The desycing can only be compensated in software by drawing OBJs one frame ahead of SCROLLs.
...
Up to that point, the frustration spur from sprite dimensions (all sprites had to have the same sizes), shapes (mandatory rectangular) and colors (one palette per sprite). 
...
Alike the other layers, OBJ palette page features 32 units which any tile can freely use.

\pagebreak

Street Fighter II embodies the power of the tile system. Combining them creates a universe of rich characters with specific shapes, giving them more personality.
...
The ultimate boss in "The Punisher", Kingpin, is a mountain of a man made of 69 tiles which covers half the screen. This impressive feat came with minimal "wasted" pixel thanks to the usage of composed tiles.
...
\end{minipage}
 ...
 When Final Fight mini-boss, Damned, does a back-flip in level 1, no rotation is performed. Two sets of tiles are used and X/Y flipped to generate two extra mirrored sets. The effect works with four poses, thanks to fast movements and players' brain interpolation.

\pagebreak

\subsection{OBJ Limitations}
The sprite system has a hard limit of 256 tiles per frame. This is not an arbitrary number since the constraint is dictated by how many tile the system is able to read from the GFXROM and write to the VRAM during a full CRT raster scan (16.7ms).
...
Street Fighter II designers pushed the machine on the edge of its limits by using OBJ tiles not only for the opponents, arena decoration, GUI, but also to embellish the background parallax effect. This led to problems when a sequel was in the making.

When Ken faces Ryu in Japan, nearly 200 tiles are used. If two of the biggest contestant, Honda and Zangief, were to face each other on this stage, the CPS-1 would be unable to render all OBJ tiles necessary. Such configuration was impossible in "Street Fighter 2" but became a feature of "Street Fighter 2: Champion Edition"  which allowed mirror opponents to face each other on any location.
\vfill
...
\begin{trivia}
Decoration in Street Fighter 2 were the object of much consideration. The stone on the ground in Sagat's stage randomly move at the beginning of each round so they cannot be used as landmarks by players.
...
\begin{itemize}[topsep=0pt]
\item Two heroes Haggar and Cody.
...
Haggar, the sprite on the list, pushes the total 2 tiles past the maximum, all it tiles are dropped.
...
The lack of OBJ scaling and rotation was a problem for developers of Street Fighter 2 where the intro needed precisely these two operations. 
...
The OBJ system was used in a creative way by Akiman, the artist planner on Street Fighter 2 at the time, to solve a show-stopper bug.
...
Akiman found that in one of Guile's pose, his calf met these criteria with a simple pen value in its lower left.
...
\nbdraw{world_warrior_title}
 \caption*{18 draws calls. Three more than necessary but with proper grammar.}%
 \end{figure}%
...
\subsubsection{VRAM}
The VRAM system is physically split via two independents blocks A and B to facilitate sprite buffer double buffering. This component also benefit from exceptionally powerful chip compared to the rest of the machine.

A quick glance at the \icode{HM53461P} shows the usual \icode{+5V}, \icode{GND}, \icode{CLK}, and address/data pins. However \icode{SD0}, \icode{SD1}, \icode{SD2}, and \icode{SD3} indicate this chip does more than the one we have seen so far.
...
This design would allows RAM and SAM to be accessed simultaneously but this never happen since when A is read, B is written and the other way around). The real value is in the access time. If the RAM port's 100ms is "normal" figure, the SAM port read operations complete more than twice as fast, taking only 40ms. 
 ...
 On the Street Fighter II board, twelves \icode{HM53461P} are combined into six pairs, resulting in 384 KiB. Four chips are used for a single line so 96 KiB is never used.
...
While other chips on the board-B are \icode{27C010} and \icode{27C512}, the GFXROM is made of \icode{MB834200B} (256 Ki x 16-bit). This type of ROM have a much higher capacity but also a slower access time (150ns). 
...
\textbf{Hardware piracy} means selling physical copies of PCBs (called bootlegs). By dumping the ROMs content from a legitimate board, buying the same off-the-shelf components, pirates sold the same game for cheaper.
...
\textbf{Software piracy} involved operators purchasing an authentic game to get the PCBs but then copying ROMs and get newer games for free. 

Capcom's response was a two-way verification where the hardware could be actively used by the software to check the board authenticity backed by "passive" mechanisms that allowed the hardware to check the software behavior.
...
The heart of the protection system is the CPS-B. The core idea is to make it behaves differently depending on the game it is supposed to run.
...
\subsection{ID check}
The simplest copy protection available is the chip ID check. By polling a register, the m68k prompts the CPS-B to return its version number. A version matches let the code know if its belongs to the right PCB and reset the CPU if it doesn't. 

To make instructions patching of the 68000 ROMs more difficult, calls to verify the chip ID are placed in several location in the code. Motivated programmer tried anyway\cite{strider_conversion}!

\subsection{Multiplication check}
Starting with CPS-B v21, a slightly more robust feature gave the CPS-B the ability to perform multiplications. Two registers are written and a third one can be read. The 68000 code checks that the returned multiplication result is the expected value.

\subsection{Moving registers}
The CPS-B registers move between revisions. The offset and range the CPS-B mapping does not change, but the offset of each register inside that mapping are different. Accessing the scroll control, scroll priority, and palette upload registers is done slightly differently for each game.

Additionally, the meaning of each bit field inside each register is altered between version.



\subsection{Unexpected behavior detection}
Protection described so far involved an active software and a passive hardware. The hardware can also actively monitor the software by leveraging the moving registers policy. If the CPS-B detects incoherent values written to the wrong registers, it sets and lock all palettes of all layers to black. 

The game still runs in the background and the audio is playing but the screen doesn't display anything. The only way to recover is to reboot the machine\cite{petitSecurity}... only for the screen to turn black again.
...
The whole behavior is encoded in a small internal 18 bytes area using not ROM but RAM. To keep these bytes alive, the CPS-B v21 must be supplied with current at all time\cite{petitSecurity}.
...
\begin{trivia}
These battery worked remarkably well since, thirty years later, one can find boards in working condition still using their original battery.
\end{trivia}
...
The first method is called "phoenix"-ing. It is a tedious process which consist in dumping a game ROM and patch the m68k instructions to replace CPS-B registers accesses to use the "default values"\cite{csp1_phoenix}. 
...
Eventually, passionate people figured out the process of accessing the CPS-B RAM and write it. Boards can be brought back to life by re-uploading the proper configuration bytes which essentially de-suicides them\cite{arcadeHackerCPS1Desuicide}.
...
\section{Epilogue}
From 1988 to 1995, Capcom used the CPS-1 to release more than thirty titles. These seven years saw the birth of three of Capcom most loved franchise, Ghouls’n Ghosts, Final Fight, and Street Fighter 2.

To Capcom, the CPS-1 was a gamble that paid off hundredfold, allowing them to become a video-game household. 

To players, the games were a series of beautifully crafted titles which both provided entertainment and emptied their pockets. The experience was so memorable that passionate people wrote emulators and even, in some extreme cases of obsessions, books, to keep these memories alive. 

To counterfeiters, the CP-System was a problem. Capcom games were popular and generated a substantial amount of money. It is likely greed set in even more so once the demand skyrocketed with the advent AAA titles such as Street Fighter II. 
...
The money in the balance armed the counterfeiters with an unreasonable amount of tenacity. As players lined up to spend time and beat Street Fighter 2, so did the pirates to defeat the copy-protection systems. Eventually they were able to figure out. 
...
As cracks appeared in its shield, Capcom did not give up on protecting its tiles. As it had proved itself able to evolve and compete in the business of producing games, it embraced the challenge to embark on an encryption crusade against bootlegers.
...
\label{kabuki}
The CPS 1.5 is noteworthy for its improved copy-protection where the audio instructions are stored encrypted in the ROM. The audio CPU is a special z80 dubbed Kabuki\cite{arcadeHackerKabuki} able to decrypt instructions on the fly.
...
The key is not burned in the z80's silicon but, alike the CPS-B v21 configuration, stored in an internal RAM. To keep that key alive, the unused pin 28 we saw on page \pageref{z80_pinRFSH} is re-purposed from "DRAM refresh" to providing power. Alike the CPS-B and its RAM configuration, the z80 requires to be powered at all time which means the system was provided with a second "suicide battery". 
...
In terms of copy-protection, Capcom once again cranked up security. Not only the platform featured the Kabuki audio instruction encryption scheme, it also gained encrypted game logic. 

Thanks to a custom CPU, ABI compatible with the 68000, instructions are stored encrypted in ROM and decrypted on the fly. Alike Kabuki, the scheme uses a shared secret key that stored in yet another battery-powered RAM (the third one).

The only part of the CPS-2 that remained stored in plain was the OKI ROM containing audio samples. The GFXROM data was not encrypted but its content was obfuscated via shuffling.

\begin{trivia}
No bootleg of CPS-2 title were ever produced. Its protection system was remarkably strong and held for nearly 10 years. It was only when the "CPS-2 Shock Group" attacked it in 2003\cite{cps2rebirth} that its functioning was reverse engineered.
\end{trivia}
