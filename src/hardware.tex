...
Even a summary glance at the Board-B will reveal that the chips are not like the others. Where we find \icode{27C010} and \icode{27C512} everywhere else, the GFXROM is made if \icode{ MB834200B-15} (256 Ki x 16-bit). To make up for the rather slow access time (150ns), they are arranged in a four-way interleave resulting in a 64-bit data path.

\icode{BYTE} pin allows to select H or L byte.
\pagebreak
