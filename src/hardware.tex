...
To craft a video system means to build a signals generator and a color generator. We'll study the signal generator first. This circuit is built to take an oscillator signal as input and to output three signals. One signal tells for how long the color generator should hold a color on the color lines, one signal triggers HSYNC, and one signal triggers VSYNC. 
...
Before looking at Capcom choices, let's look at the decisions made by the video designer for systems contemporary to the CP-System. 
% e.g: The Neo-Geo entry shows a machine using a resolution of \icode{384x264} and a dot-clock of 6 Mhz which means it issues a color every $\frac{1}{6,000,000}\mathtt{th} $ of a second.
...
Lastly, the video system needs break to read or write data without generating visible artifacts. This includes swapping buffers, updating the palettes colors, and retrieving the list of sprites/tilemap to draw on next scanline.

The solution to these three problem is named blanking. By setting the color signals to zero, the cannon shoots no electrons, artifacts are hidden and a timespan where the video system is inactive is created. There is a vertical blanking called VBLANK and an horizontal blanking called HBLANK.


\subsubsection{Capcom second set of video signal choices}

Out of these 262 lines available, Capcom decided to use only 224 and let VBLANK last for $\mathtt{262 - 224 = 38}$ lines. Likewise, they kept 384 dots usable per lines and gave $\mathtt{512 - 384 = 128}$ to HBLANK. These left the machine with a resolution of \icode{384x224}.
...
\begin{tabularx}{\textwidth}{Xrrrr} 
  ...
  \toprule    
   ...
   \textbf{HBLANK (dots )} & \texttt{128}  & \texttt{?} & \texttt{64} & \texttt{85}  \\
   \textbf{Usable lines} & \texttt{224} & \texttt{224} & \texttt{224} & \texttt{224} \\
   \textbf{VBLANK (lines)} & \texttt{38}  & \texttt{?} & \texttt{40} & \texttt{38}  \\
\toprule    
...
The scanlines of a CRT have a fixed height. The dot-clock dictates the width of each dot. Together they define a width/height dot ratio called the Pixel Aspect Ratio (PAR).

All CRTs at the time were built with a physical scanline height guaranteeing that a dot clock of 6,136,363 Hz ($\frac{135}{22}$) would generate "square" dots with a 1:1 PAR. A system using a higher frequency would draw narrower dots while a system using a lower frequency would draw wider dots.
...
The Neo-Geo MVS, with its dot-clock of 6,000,000 Hz has a PAR of 45:44. Combining its \icode{320x240} format gives the Display Aspect Ratio (DAR) of the physical image seen on the CRT.
...
The CPS-1 with its resolution of \icode{384x224} and its dot-clock of 8,000,000 Hz results in a PAR of 135:176. Its DAR somewhat matches the CRT aspect ratio of 4:3. 
