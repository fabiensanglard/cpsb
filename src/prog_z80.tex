...
Graphing PCM vs ADPCM shows that the decompressed stream "lags" behind upon abrupt changes but catches up aggressively. ADPCM first value is always near zero (+/-28) but it is not audible to players since most samples are faded in anyway. 
...
\lstinputlisting{src/code/okistruct.c}
...
In function \icode{schedInterrupt} we write to the YM2151 counter register msb 8-bit \icode{0xC0} and the 2 remaining lsb \icode{0x00}. Notice how before writing to the YM2151 register we must make sure the chip is not busy via \icode{waitYM2151()}.
...
To finish bootstrapping we make sure initalized C variables are indeed initialized. The linker placed all these values in a \icode{\_GSINIT} segment. By placing markers astutely at the source \icode{\_INITIALIZER} and at the destination \icode{\_INITIALIZED}, they can be copied.
...
\lstinputlisting{src/code/z80/music.c}


\lstinputlisting{src/code/z80/driver.c}
