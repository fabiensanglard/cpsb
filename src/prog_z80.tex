...
ADPCM encodes the difference between the previous sample and the sample to be generated. This is done via a 4-bit magnitude nibble. The meaning of the magnitude value depends on the state of the decoder which features a \icode{step size}.
...
ADPCM starting step size index is \icode{0} which translate in the \icode{stepTable} as \icode{16}.

\lstinputlisting[language=C]{src/code/adpcm_compression1.c} 

The first value to compress is shifted from 16-bit to 12-bit. The 
