...
With the registers correctly mapped, what remains is to place the \icode{.text} and \icode{.data} at the right location in ROM. This can easily be done thanks to the awesome \icode{sdldz80} linker and its linker script.
...
To finish bootstrapping, \icode{crt0} makes sure initialized C variables values are set. The linker placed all values requiring initialization in a \icode{\_GSINIT} segment. By wrapping it with markers \icode{\_INITIALIZER} (src) and \icode{\_INITIALIZED} (dst), they can be copied easily.
...
The \icode{main} uses an "active" loop in order to never outpace be \icode{interrupt} function call frequency. This guarantees it runs in the vicinity of 250Hz. 
