...
The compiler in the GNU Compiler Collection, \icode{gcc} does not have a placement keyword \icode{\_at} but even if it had been available, it would not have been enough to map large portions such as the 192 KiB of GFXRAM.
...
\begin{trivia}
In Street Fighter 2, DIP B is used to configure the difficulty level of the game ranging from 0 to 8 (4=default). Based on this value the A.I selects appropriate sets of bytecode\cite{sf2aiengine} script. Even in easy mode, the A.I cheats by skipping "charging"\cite{sf2aiengine}.
\end{trivia}
...
Since developers detailed to what extent SHARP's machine was involved, we can only make an educated guess. A beginning of an answer comes from the hardware components, while Capcom-produced software leaves no ambiguity.
...
Lastly, the X68000 floppy storage resulted in slow loading time compared to the CPS-1 ROM. To solve this issue, developers used as much RAM as available, going as far as loading the full content of the floppies during startup if the capacity of the X68000 allowed it.
...
The XM6 Pro-68k emulator and a methodology shared by Upsilandre\cite{x68k_games_analysis} (detailed in the next pages) shows that the GFX rendering architecture of these titles exhibits no pattern of an emulation layer. Each GFX renderer is tailor made to rely heavily on the CPU. 

The stress on the 68000 is confirmed by Ghouls 'n Ghosts manual which recommends a 16MHz CPU and warns about slowdown with a machine using a 10MHz 68000. This leaves little doubt to the limited usage of the X68000 to write and test assembly, run TCE, and allocate/compile the GFXROM for CPS-2 games.
...
The Sprite layer contains more than sprites. While rendering the wind blowing in the grass only required swapping tile ID on CPS-1, the X68000 could not plot that many pixels in the Graphic planes. Promoting blades to sprites overlays reduced the fillrate.
...
The graphic render uses the same trade off used by Ghouls 'n Ghosts where usage of Tilemaps is sacrificed in favor of the ability to feed the Sprite layer with tiles. 
...
The color-depth reduction trick is only one among many others. There are many other testament to the hair pulling process it must have been to port to the X68000. One of them is the introduction sequence where Damned takes away the mayor's daughter, Jessica Haggar.

This scene features many sprites tiles, among them two heroes, the mini-boss Damned, Jessica, two minions (XXXX and YYYY), and six barrels. The number of tiles far exceed the maximum 128 limit of the X68000.
...
To manage this problem, developers started by cutting out one minion (). 

Since it was still not enough, they resorted to using "soft sprites". When the sprite layer is full, additional sprite tiles are drawn in software in the Graphic Plane Page 1. 

This trick should have brought the m68k CPU to its knees when the barrels are destroyed. When they explode in multiple pieces, the CPU would have had to repeatedly delete the dirty section of Graphic Plane before drawing the moving parts of the barrel. 

We won't even get into the obscene amount of overdraw incurred when parts of a barrel overlap with each others.
...
The trick is to move the barrels form the Background to the Sprite layer as enemies flee. The second player still disappears due to tile scarcity when the barrels are destroyed.
