...
It is considered a "Perfect port" because of its GFX faithfulness to the CPS-1 version even though its graphic renderer is completely different. Tilemaps are not used at all since the background lives in two 512x512 Graphic layer with shared 8-bit indexed colors.
...
Notice the vertical "cut" in the right of Graphic plane Pages 0 and 1. Thanks to the hardware scrolling wraparound, only new portions of the screen had to be software rendered. 
...
Released in 1992, Final Fight uses a similar architecture as Ghouls 'n Ghosts. It did a good job but the number of enemies on screen had to be lowered.

The stair trick is achieved by using a special blending mode where the MSB indicates precedence over Sprite layer. As a result the Background uses only 128 colors instead of the 256 possible.

\nbimg{x68k_ff_scr.png}



\nbimg{x68k_ff_text.png}
  
  \nbimg{x68k_ff_bitmap0.png}

  \nbimg{x68k_ff_bitmap1.png}
  ...
  \nbimg{x68k_ff_sprites.png}




\subsubsection{Ports Analysis: Street Fighter II Champion Edition}
