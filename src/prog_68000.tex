...
While the latch toward the Sound system is a small API surface, the GFX API is huge. The interface to the Grapic System is bigger both in breadth (it features 64 registers) and depth (the expected GFXRAM data layout is non-trivial).
...
At the end of the process, the logical 68000 ROM is split and interleaved in chip sized ROMs according to what was described in the hardware chapter.
 ...
 Alike for the z80, bootrapping is solved with a small assembler programm named \icode{crt0.s}.
...
A simpler mode, auto-vector, makes the CPU jump directely based on the three IPL lines state. Three lines gives a value [0,7] which is used to looked up the "vector table" starting at offset 24.
...
Faulty RAM could create issues further down the pipeline. A cabinet would then expose faults that could be misattributed to other components. By allowing to quickly rules out a whole class of errors, the startup tests sped up debugging and repairing boards.
...
The architecture is much alike the Z-80 sound system where two "threads" run in lockstep. Function \icode{VSync} is awakened roughly every 16ms. Its job is to read inputs and save them locally, read latch buffer and write them to the latch, and more importantly, flip the GFXRAM doube buffered descriptors. 
...
However for the simplication and brievty requirement of this book, no queue is used in the following code.
...
The proper way to avoid visual artifacts is to us a double buffer in the GFXRAM where the currently used data is not written until the next VSYNC. Meanwhile the next frame is being prepared in the other buffer. 
...
Rowscrolling allows to offset each visible row via a discret X amounts. In Street Fighter II, Honda's dohyō ( the space in which a sumo wrestling bout occurs) perspective is achieved via linear offset differences. The more perspective needed, the more accentuated is the offset slope. 
...
\lstinputlisting[language=C]{src/code/68000/rowscroll.c} 
...
The series is largely unknown outside of Japan. But in that country it is known as the "god computer". 
...
Besides its gorgeous "Manhattan" case, the machine packed a surprising amount of horse power. Its graphic system supporting 128 hardware 16x16 tiles was so impressive that it surpassed dedicated arcade hardware capabilities at the time.
...
The similarities with the CPS-1 are troubling. It seems like a software emulation layer would make it a perfect CPS-1 development machine. However if we look into the details, things are different.
...
The hardware is so powerful that developers were able, for some games, to release "perfect port" which were visually identical to the arcade version. Capcom's Ghouls 'n Ghosts and Street Fighter 2 are considered perfect whereas Final Fight had to lower the number of enemies on screen.

 SHARP kept on improving the series with better CPU (68030), more RAM (up to 4MiB) and even HDD (up to 80 MiB). Peripheral manufacturers embraced the computer with extensions cards covering anything one can thing off. 
...
The success was such that an x68000 dedicated magazine, Oh!X, was published from 1988 to 2000 over 139 volumes. 
...
Often featuring a raytraced cover to boast the graphic capabilities of the SHARP marvels, each publication came loaded with softwares, originally on one, then two, and later three 5.25" floppy disks!
