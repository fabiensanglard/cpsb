...
To mark the end of the tile list an empty descriptor is used with attribute value set to \icode{0xFF00}. 
...
Next to no information has ever transpired regarding how the CPS-1 control system was programmed. However one machine is widely rumored to have been used. A rumor seemingly confirmed by the nickname of Hiroaki Kondo, a Capcom composer/sound programmer often credited as "X68K". A rumor further renforced by the components found inside the machine, which closely match the CPS-1.
...
The hardware in these workstation were to put it midly, mind blowing at the time. A bet that was especially risky from SHARP considering the machine has next to no software at launch since it used its own OS.
...
CPU & Motorola 68000 XX MHz \\  
RAM &  \\ 
Video  & \\
Graphics & \\
Sprite & 128 tiles total onscreen. 32 tiles per line.\\
VRAM & Storing 256 16x16 tiles\\
Sound chip & Motorola 68000  \\ 
Music chip & Motorola 68000  \\ 
...
Most importantly, the Sprite system has a weakness. The number of tile displayable was half the capacity of the CPS-1 (128 instead of 256) but that was an issue easily circuveined thanks to techinques such as Sprite doubler by Koichi Yoshida\cite{x68000spritedoubler} relying on raster effect.
...
These limitation did not seem to have deterred Capcom from releasing five games for the platform (Ghouls 'n Ghosts, , Final Fight, SF2CE, Super Street Fighter II, and Strider). 

Talk about perfect ports (SF2) and non-perfect port like Final Fight, the issue likely not being the number of sprite displayble but rather the VRAM that allowed only 256 different tiles.

Over its livespan, the X68000 series was immenseily succesful in Japan with 823  games released from 1988 to 1999 and even 2004.

Many peripherals avaialble.
...
Fascination around this marchine is such that functioning model sell for \$2000 nowadays.

Further analysis of the GFX sheet show that an allocator was used -> Workstation?

\img{SharpX68000ad.jpg}

\begin{trivia}
In France, SHARP was a brand known for selling excellent things that noone would ever be able to afford. It became a national sensation when Eric Cantona made the commercial "Précis, pointu, affuté !"
\end{trivia}
