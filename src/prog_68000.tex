...
The sprite system is also, at first sight, weaker since the number of tile displayable is half the capacity of the CPS-1 (128 instead of 256). However that is an issue easily circumvented thanks to multiplexing techniques such as Sprite doubler by Koichi Yoshida\cite{x68000spritedoubler} relying on raster effect.\index{Sprites Multiplexing!X68000}
...
\subsection{32 KiB VRAM wall}

The real issue for all Capcom ports was the amount of VRAM available to the sprite and tilemap systems. Where a CPS-1 game ships with a minimum of 2MiB ROM storing the GFX assets, the X68000 can only uses 32KiB out of the 1MiB of VRAM for the same purpose. 

This small amount effectively keep-capped the idea of using both the Sprite and Tilemap system simultaneously since one of them would starve for tiles. 

\subsection{Learning from X68000 ports}

The limited amount of VRAM prevented the X68000 from being a CPS-1 emulator and likely relegated it to author/test 68000 ASM, run a port of TCE for GFX integration, and allocate/compile the GFXROM for CPS-2 games. 
