...
When a contestant in Street Figther II jumps, SCROLL2 moves vertically but no rowscroll offsets are re-calculated, only \icode{ROWSCROLL\_OFFSET}  is moved. This allows the engine to amortize the generation of perspective correct rowscroll offset.
...
The board studied in this book uses a CPS-B 11 which does not support STAR1 and STAR2. These layers were also marginally used in Forgotten Words and Strider (two good reasons to allow the author to avoid detailing the STARs bytecode further).
