...
The compiler in the GNU Compiler Collection, \icode{gcc} does not have a placement keyword \icode{\_at} but even if it had been available, it would not have been enough to map large portion such as the 192 KiB of GFXRAM.
...
The color-depth reduction trick is only one among many others. There are many other testimony to the hair pulling process it must have been to port to the X68000. One of the them is the introduction sequence where Damned takes away the mayor's daughter, Jessica Haggard.
...
This trick should have brought the m68k CPU to its knees when the barrels are destroyed. Because they explode, the CPU would have had to repeatedly delete the dirty section of Graphic Plane before drawing the moving parts of the barrel. 
...
\vspace{4ex}
Final Fight X68000 only display 
...
Alike in Final Fight, where scenes were optimized on a case-per-case basis, Street Fighter II Champion Edition uses a distinct rendering strategy depending on the arena.
